-- Personal config for shortcuts, window rules, startup applications and workspaces
...
-- Left top window: Launches fastfetch after 1 second
hl.exec_cmd("sleep 1 && ghostty --title=fastfetch-boot -e zsh -c 'fastfetch; sleep 2 && echo \"\\n--- Tailscale Status ---\\n\"; tailscale status; exec zsh'")
-- left bottom window: launches btop after 2 seconds
hl.exec_cmd("sleep 3 && exec ghostty --title=btop-boot -e btop")
-- right large window: launches blank ghostty terminal after 3 seconds
hl.exec_cmd("sleep 4 && ghostty --title=ghostty-boot ")
...
-- workspace 1 no gap, no rounding, min border
hl.workspace_rule({ workspace = "1",   gaps_out = 0, gaps_in = 0 })
hl.window_rule({
     name  = "no-gaps-w1",
     match = { float = false, workspace = "1" },
     border_size = 0,
     rounding    = 0,
})
-- hl.window_rule({
